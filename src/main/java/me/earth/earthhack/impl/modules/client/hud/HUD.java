...
public class HUD extends Module {
    ...
    protected void renderModules() {
        ...
        if (tps.getValue()) {
            String tps = "TPS " + TextColor.GRAY + MathUtil.round(Managers.TPS.getTps(20), 2);
            String newerTps = TextColor.DARK_GRAY + " [" + TextColor.GRAY + MathUtil.round(Managers.TPS.getTps(5), 2) + TextColor.DARK_GRAY + "]";
            String toDraw = tps + newerTps;

            renderText(toDraw, width - 2 - RENDERER.getStringWidth(toDraw), height - 2 - RENDERER.getStringHeight() - offset - animationY);
            offset += RENDERER.getStringHeight() + 3;
        }
        if (fps.getValue()) {
