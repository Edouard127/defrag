...
implements Util {
    ...
    public static Vec3d getInterpolatedRenderPos(Entity entity, float partialTicks) {
        Vec3d subtract = EntityUtil.getInterpolatedPos(entity, partialTicks).subtract(EntityUtil.mc.getRenderManager().renderViewEntity.prevPosX, EntityUtil.mc.getRenderManager().renderViewEntity.prevPosX, EntityUtil.mc.getRenderManager().renderViewEntity.prevPosZ);
        return subtract;
    }

    public static Vec3d getInterpolatedRenderPos(Vec3d vec) {
        return new Vec3d(vec.x, vec.y, vec.z).subtract(EntityUtil.mc.getRenderManager().renderViewEntity.prevPosX, EntityUtil.mc.getRenderManager().renderViewEntity.prevPosX, EntityUtil.mc.getRenderManager().renderViewEntity.prevPosZ);
    }
