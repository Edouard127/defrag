...
object ElytraFlight : Module(
    ...
    private fun SafeClientEvent.landing(event: PlayerTravelEvent) {
        when {
            ...
            LagNotifier._paused && LagNotifier.pauseTakeoff -> {
                holdPlayer(event)
            }
    ...
    private fun SafeClientEvent.takeoff(event: PlayerTravelEvent) {
        ...
        if (!easyTakeOff || (LagNotifier._paused && LagNotifier.pauseTakeoff) || player.onGround) {
            if (LagNotifier._paused && LagNotifier.pauseTakeoff && player.posY - world.getGroundPos(player).y > 4.0f) holdPlayer(event) /* Holds player in the air if server is lagging and the distance is enough for taking fall damage */
